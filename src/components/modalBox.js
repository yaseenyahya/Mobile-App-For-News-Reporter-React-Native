...
export default class ModalBox extends React.PureComponent {
  ...
  refresh = entry => {
    ...
    var position = new Animated.Value(
      entry === "top" ? -SCREEN_HEIGHT : SCREEN_HEIGHT
    );
    this.setState({ position, pan: this.createPanResponder(position) });
  };
  constructor(props) {
    super(props);
